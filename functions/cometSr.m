...
if nargs < 4, plotPause = .01; end
if nargs == 4, [x,y,p,plotPause] = deal(args{:}); end
if nargs < 5, plotColor = []; end
if nargs == 5, [x,y,p,plotPause,plotColor] = deal(args{:}); end
...
if isempty(plotColor)
    ...
    head = line('parent',ax,'color',plotColor,'marker','o','linestyle','none', ...
        'xdata',x(1),'ydata',y(1),'Tag','head','lineWidth',2);
    body = matlab.graphics.animation.AnimatedLine('color',plotColor,...
        'linestyle',lstyle,'lineWidth',2,...
        'Parent',ax,...
        'MaximumNumPoints',max(1,k),'tag','body');
    tail = matlab.graphics.animation.AnimatedLine('color',plotColor,...
